...
<div class="p-6 bg-white rounded-lg shadow max-w-5xl mx-auto relative">
    ...
    <div class="w-full text-center mb-6">
        <h2 class="text-xl font-semibold text-gray-700 inline-block">
            NOTA DE SALIDA MATERIAL - {{ $nota->codigo }}
        </h2>
    </div>
        ...
        <table class="w-full text-sm text-left text-gray-700 border border-gray-200 rounded-lg">
            <thead class="bg-gray-100 text-gray-600 uppercase">
                <tr>
                    ...
                    <th class="px-4 py-3 border">Unidad</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($nota->detalles as $detalle)
                    <tr class="border-b hover:bg-gray-50">
                        <td class="px-4 py-2 border">{{ $detalle->material->nombre ?? 'N/A' }}</td>
                        <td class="px-4 py-2 border">{{ $detalle->cantidad }}</td>
                        <td class="px-4 py-2 border">{{ $detalle->unidad_medida }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <div class="mt-6 text-right">
        <a href="{{ route('materiales.notasalida.index') }}"
